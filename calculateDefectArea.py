...
def calculate_defect_area(image_path, csv_folder):
    ...
    for file_path in csv_path_list:
        defect_map, num_non_probe_area = csv_to_defect_map(image=image, csv_path=file_path)
...
def csv_to_defect_map(image, csv_path):
    ...
    if defect_type == DefectType.CHIPPING:
        defect_map = calculate_defect_map_chipping(coordinates, image_resized, threshold=0.9)
    if defect_type == DefectType.WHISKERS:
        defect_map = calculate_defect_map_whiskers(coordinates, image_resized, threshold= 0.1)
