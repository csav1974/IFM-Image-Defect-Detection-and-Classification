...
def count_defects(
    image_path, threshold_value=50, max_value=255, margin=5, min_area=0, max_area=500
):
    # Laden des Bildes
    ...
    _, binary_image = cv2.threshold(
        roi, threshold_value, max_value, cv2.THRESH_BINARY_INV
    )
...
def select_files_and_process():
    ...
    results_file = os.path.join(
        os.path.dirname(file_paths[0]), "defect_count_results.txt"
    )
    ...
    with open(results_file, "w") as f:
        for file_path in file_paths:
            num_defects, dead_area_ratio, marked_image, error = count_defects(
                file_path, threshold_value, max_value, margin, min_area, max_area
            )
            ...
            if error:
                ...
                f.write(
                    f"{file_name}: Anzahl der Defekte - {num_defects}, Dead Area Ratio - {dead_area_ratio:.4f}\n"
                )
                # Speichern des markierten Bildes (optional)
                marked_image_path = os.path.join(
                    os.path.dirname(file_path), f"{file_name}_marked.png"
                )
